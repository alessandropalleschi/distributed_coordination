...
if __name__ == '__main__':
    env = MultiRobotEnv(num_obstacles=0,num_robots=1)
    N = 20
    batch_size = 50
    n_epochs = 20
    alpha = 0.0003
    ...
    for i in range(n_games):
        ...
        while not done:
            ...
            observation_, reward, dones, truncated = env.step(action)
            done = np.logical_or(dones,truncated)
            n_steps += 1
            score += reward
            ...
            for robot_id in range(env.num_robots):
                ...
                if truncated[robot_id] or dones[robot_id]:
                    last_value = agent.compute_value(observation_)
                    agent.store_last_value(last_value[robot_id], robot_id)
